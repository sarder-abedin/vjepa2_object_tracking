...
Conda:
`conda create -n vjepa2-tracker python=3.10 -y`
`conda activate vjepa2-tracker`
...
This project has its own [`Dockerfile`](Dockerfile) (Python 3.10-slim + the system libraries OpenCV needs for its GUI window and webcam capture). It builds a Linux-based image that runs identically through Docker on **Linux, macOS, and Windows** hosts — what differs between hosts is how you grant the container access to your webcam and display (see below).

### 1) Build the image

```bash
cd patch_tracker
docker build -t vjepa2-patch-tracker .
```
...
- **Run the script directly on the host** (see *Installation* above) — simplest for webcam access.
